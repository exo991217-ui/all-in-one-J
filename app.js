...
function renderSettings(){
  ...
  container.innerHTML=`
    ...
    <!-- ✈️ 여행 플래너 분류 설정 -->
    <div class="travel-settings-section" style="margin-top:24px;padding-top:24px;border-top:2px solid #E8F5EE;">
      <h3 style="font-size:15px;font-weight:800;color:#1a1a2e;margin-bottom:18px;display:flex;align-items:center;gap:8px;">✈️ 여행 플래너 분류 설정</h3>
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:16px;">
        <div class="cat-manage-section" style="background:#F0F9F4;border-radius:14px;padding:16px;border:1.5px solid #E8F5EE;">
          <div style="font-size:13px;font-weight:700;color:#4CAF82;margin-bottom:10px;">📅 여행일정 분류</div>
          <div class="cat-tags" id="cat-tags-schedule" style="margin-bottom:10px;"></div>
          <div class="cat-add-row" style="display:flex;gap:6px;">
            <input type="text" id="cat-input-schedule" placeholder="새 분류..." style="flex:1;padding:7px 10px;font-size:0.8rem;border-radius:8px;border:1.5px solid #e5e7eb;" onkeydown="if(event.key==='Enter')addCategory('schedule')" />
            <button style="padding:7px 14px;border-radius:8px;border:none;background:#4CAF82;color:#fff;font-size:0.8rem;font-weight:700;cursor:pointer;" onclick="addCategory('schedule')">추가</button>
          </div>
        </div>
        <div class="cat-manage-section" style="background:#F0F9F4;border-radius:14px;padding:16px;border:1.5px solid #E8F5EE;">
          <div style="font-size:13px;font-weight:700;color:#4CAF82;margin-bottom:10px;">💸 지출 분류</div>
          <div class="cat-tags" id="cat-tags-expense" style="margin-bottom:10px;"></div>
          <div class="cat-add-row" style="display:flex;gap:6px;">
            <input type="text" id="cat-input-expense" placeholder="새 분류..." style="flex:1;padding:7px 10px;font-size:0.8rem;border-radius:8px;border:1.5px solid #e5e7eb;" onkeydown="if(event.key==='Enter')addCategory('expense')" />
            <button style="padding:7px 14px;border-radius:8px;border:none;background:#4CAF82;color:#fff;font-size:0.8rem;font-weight:700;cursor:pointer;" onclick="addCategory('expense')">추가</button>
          </div>
        </div>
        <div class="cat-manage-section" style="background:#F0F9F4;border-radius:14px;padding:16px;border:1.5px solid #E8F5EE;">
          <div style="font-size:13px;font-weight:700;color:#4CAF82;margin-bottom:10px;">⭐ 버킷플레이스 유형</div>
          <div class="cat-tags" id="cat-tags-bucket" style="margin-bottom:10px;"></div>
          <div class="cat-add-row" style="display:flex;gap:6px;">
            <input type="text" id="cat-input-bucket" placeholder="새 유형..." style="flex:1;padding:7px 10px;font-size:0.8rem;border-radius:8px;border:1.5px solid #e5e7eb;" onkeydown="if(event.key==='Enter')addCategory('bucket')" />
            <button style="padding:7px 14px;border-radius:8px;border:none;background:#4CAF82;color:#fff;font-size:0.8rem;font-weight:700;cursor:pointer;" onclick="addCategory('bucket')">추가</button>
          </div>
        </div>
      </div>
      <p style="font-size:11px;color:#6b7280;margin-top:12px;">✈️ 여행 탭의 일정·지출·버킷플레이스 입력 폼에서 사용할 분류를 관리합니다.</p>
    </div>
  `;
  // 설정 탭에서 여행 분류 태그 렌더링
...
// ============================================================
// ✈️ 여행 플래너 — 통합 모듈 (나만의 가계부 + 여행을 떠나자)
// ============================================================

// Firebase refs (shared with budget app via firebase-sync.js)
let travelDb = null;
let travelAuth = null;
let travelCurrentUser = null;

// Auth callback from firebase-sync.js
window.onTravelAuthChange = function(user) {
  if (!travelDb && window._travelDb) travelDb = window._travelDb;
  if (!travelAuth && window._travelAuth) travelAuth = window._travelAuth;
  travelCurrentUser = user;
  const c = document.getElementById("trips-container");
  if (user) {
    if (c) loadTrips();
    loadAllBucketItems();
    if (document.getElementById("tab-travel") && document.getElementById("tab-travel").classList.contains("active")) calcStorage();
  } else {
    if (c) c.innerHTML = '<div class="empty-state"><div class="icon">✈️</div><h3>로그인이 필요해요</h3><p>Google 계정으로 로그인 후 여행 기록을 시작하세요</p></div>';
  }
};

// Travel-specific modal helpers (travel modals use .hidden class)
function openTravelModal(id) { var el = document.getElementById(id); if (el) el.classList.remove("hidden"); }
function closeTravelModal(id) { var el = document.getElementById(id); if (el) el.classList.add("hidden"); }

// Travel sub-page navigation (within #tab-travel section)
function showTravelPage(page) {
  document.querySelectorAll(".travel-page").forEach(function(p) { p.classList.remove("active"); });
  var target = document.getElementById("travel-page-" + page);
  if (target) target.classList.add("active");
  document.querySelectorAll(".travel-subtab").forEach(function(t) { t.classList.remove("active"); });
  var subtab = document.getElementById("travel-subtab-" + (page === "trip" ? "home" : page));
  if (subtab) subtab.classList.add("active");
  if (page === "home") { if (travelDb && travelCurrentUser) { loadTrips(); renderPlanList(); } }
  if (page === "bucket") {
    if (travelDb && travelCurrentUser) { loadBucketSortState(); loadAllBucketItems(); }
    refreshCategorySelects(); updateBucketSortHeaders();
  }
}

// Travel categories rendering (called from renderSettings)
function renderTravelSettingsSection() {
  renderAllCatTags();
  refreshCategorySelects();
  if (travelCurrentUser) calcStorage();
}
...
// TRIPS — HOME
// ============================================================
function tripsRef() { return travelDb.collection("users").doc(travelCurrentUser.uid).collection("trips"); }
...
async function createTrip() {
  if (!travelCurrentUser) { showToast("먼저 로그인해주세요!"); return; }
...
// 지도 링크
// ============================================================
function openMapEntryModal() { _refreshMapEntryDisplay(); openTravelModal("modal-map-entry"); }
...
async function saveTripMapLink() {
  ...
  closeTravelModal("modal-map-entry");
  updateMapEntryBtn();
...
async function clearTripMapLink() {
  ...
  closeTravelModal("modal-map-entry"); updateMapEntryBtn(); showToast("지도 연결 해제됨");
}
...
function openMapModal(notesUrl, locationName) {
  ...
  openTravelModal("modal-map-view");
}
...
function openResLinkModal(encodedUrl) {
  ...
  openTravelModal("modal-link-preview");
}
...
// BUCKET REF
// ============================================================
function bucketRef() { return travelDb.collection("users").doc(travelCurrentUser.uid).collection("bucketItems"); }
...
function openTripBucketModal(editId) {
  ...
  refreshCategorySelects();
  openTravelModal("modal-bucket-trip");
...
async function saveTripBucketItem() {
  ...
  closeTravelModal("modal-bucket-trip"); reloadBucketAll();
}
...
function openBucketAddModal(editId) {
  ...
  if (saveBtn) saveBtn.textContent = editId ? "수정하기" : "추가하기";
  refreshCategorySelects();
  openTravelModal("modal-bucket-add");
...
async function saveBucketAddModal() {
  ...
  closeTravelModal("modal-bucket-add"); reloadBucketAll();
}
