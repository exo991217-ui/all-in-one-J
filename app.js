...
const EditLog = {
  ...
  list(contentId, tab) {
    try { return JSON.parse(localStorage.getItem(this._key(contentId, tab)) || "[]"); } catch { return []; }
  },
  snapshot(contentId, tab) {
    let data;
    if (tab === "timeline") data = Timeline.list(contentId);
    else if (tab === "sections") data = Sections.list(contentId);
    else data = Notes.list(contentId);
  ...
  restore(contentId, tab, data) {
    if (tab === "timeline") DB.set(`ff14_timeline_${contentId}`, data);
    else if (tab === "sections") DB.set(`ff14_sections_${contentId}`, data);
    else DB.set(`ff14_notes_${contentId}`, data);
  },
  openModal(contentId, tab, rebuild) {
    const tabLabel = tab === "timeline" ? "타임라인" : tab === "sections" ? "상세 공략" : "오답노트";
    const logs = this.list(contentId, tab);
    if (!logs.length) {
      showModal(`📋 수정 로그 — ${tabLabel}`,
        h("div", { style: "padding:24px;text-align:center;color:var(--muted-fg);" },
          h("div", { style: "font-size:32px;margin-bottom:8px;" }, "📭"),
          h("div", {}, "저장된 로그가 없습니다."),
          h("div", { style: "font-size:12px;margin-top:4px;" }, "탭을 이동하거나 새로고침하면 자동으로 스냅샷이 쌓여요."),
        ),
        { noCancelBtn: true },
      );
      return;
    ...
    logs.forEach((log) => {
      ...
      restoreBtn.onclick = () => {
        ...
        EditLog.snapshot(contentId, tab);
        EditLog.restore(contentId, tab, log.data);
        App.closeModal();
        rebuild();
      };
      logList.append(h("div", { style: "border:1px solid var(--border);border-radius:8px;padding:10px 12px;" },
    ...
    showModal(`📋 수정 로그 — ${tabLabel}`, logList, { noCancelBtn: true });
  },
};
...
const PinnedTabs = {
  get() {
    try { return JSON.parse(localStorage.getItem("ff14_pinned_tabs") || "[]"); } catch { return []; }
  },
  toggle(id) {
    const pins = this.get();
    ...
    localStorage.setItem("ff14_pinned_tabs", JSON.stringify(pins));
  },
  isPinned(id) { return this.get().includes(id); },
};

function renderPinnedNavTabs(currentPath) {
  const nav = document.querySelector("header .nav");
  if (!nav) return;
  nav.querySelectorAll(".nav-pinned-tab").forEach((el) => el.remove());
  const pins = PinnedTabs.get();
  const backupBtn = document.getElementById("nav-backup-btn");
  pins.forEach((id) => {
    const content = Contents.get(id);
    if (!content) return;
    const isActive = currentPath === `/content/${id}`;
    const diffLabel = content.difficulty ? ` ${content.difficulty}` : "";
    const tab = document.createElement("a");
    tab.href = `#/content/${id}`;
    tab.className = "nav-link nav-pinned-tab" + (isActive ? " active" : "");
    tab.dataset.route = `/content/${id}`;
    tab.title = content.name;
    tab.innerHTML = `<span class="pin-tab-name">${content.name}</span><span class="pin-tab-diff">${diffLabel}</span>`;
    nav.insertBefore(tab, backupBtn || null);
  });
}
...
function navigate(hash) {
  ...
  document.querySelectorAll(".nav-link:not(.nav-pinned-tab)").forEach((a) => {
  ...
  const headerEl = h(
    ...
    h(
      ...
      h(
        ...
        h("h1", { class: "detail-title", id: "detail-title-h1" }, content.name),
        content.description
      ...
      h(
        ...
        h(
          "button",
          {
            ...
            title: "제목 수정",
            onclick: () => {
              const titleH1 = document.getElementById("detail-title-h1");
              if (!titleH1) return;
              const inp = document.createElement("input");
              inp.className = "form-input";
              inp.type = "text";
              inp.value = content.name;
              inp.style.cssText = "font-size:1.5rem;font-weight:700;font-family:inherit;width:100%;max-width:500px;";
              titleH1.innerHTML = "";
              titleH1.appendChild(inp);
              inp.focus();
              inp.select();
              const save = () => {
                const name = inp.value.trim();
                if (name) {
                  Contents.update(contentId, { name });
                  content.name = name;
                  titleH1.innerHTML = "";
                  titleH1.appendChild(document.createTextNode(name));
                } else {
                  titleH1.innerHTML = "";
                  titleH1.appendChild(document.createTextNode(content.name));
                }
              };
              inp.onblur = save;
              inp.onkeydown = (e) => {
                if (e.key === "Enter") { e.preventDefault(); inp.onblur = null; save(); }
                if (e.key === "Escape") { inp.onblur = null; titleH1.innerHTML = ""; titleH1.appendChild(document.createTextNode(content.name)); }
              };
            },
          },
          "✏️",
...
function renderTimelineTab(container, contentId) {
  ...
  function rebuild() {
    EditLog.snapshot(contentId, "timeline");
    ...
    toolboxBody.append(
      h("button", {
        class: "toolbox-btn",
        onclick: () => EditLog.openModal(contentId, "timeline", rebuild),
      }, "📋 수정 로그"),
    );
    ...
    if (!entries.length) {
      ...
      entries.forEach((entry) => {
        ...
        if (editMode && entry.type !== "divider" && !locked) {
          row.classList.add("edit-mode-row");
          row.onclick = (e) => {
            if (e.target.closest("button,a,input,select,.mit-icon")) return;
            activateTimelineRowEdit(row, entry, contentId, customCols, rebuild);
          };
        }
...
function activateTimelineRowEdit(tr, entry, contentId, customCols, rebuild) {
  const stInput = h("input", { class: "inline-input", type: "text", value: entry.startTime || entry.time || "", placeholder: "0:00" });
  const etInput = h("input", { class: "inline-input", type: "text", value: entry.endTime || "", placeholder: "0:10" });
  const phInput = h("input", { class: "inline-input", type: "text", value: entry.phase || "", placeholder: "기술명" });
  const skSel = h("select", { class: "inline-select" });
  ...
  const noInput = h("input", { class: "inline-input", type: "text", value: entry.notes || "", placeholder: "비고" });
  const meInput = h("input", { class: "inline-input", type: "text", value: entry.memo || "", placeholder: "메모" });
  const customInputs = (customCols || []).map((col) =>
    h("input", { class: "inline-input", type: "text", value: (entry.custom || {})[col.id] || "", placeholder: col.name }),
  );
  const save = () => {
    const custom = {};
    (customCols || []).forEach((col, i) => {
      if (customInputs[i].value.trim()) custom[col.id] = customInputs[i].value.trim();
    });
    Timeline.update(contentId, entry.id, {
      startTime: stInput.value.trim() || entry.startTime || entry.time,
      endTime: etInput.value.trim() || undefined,
      phase: phInput.value.trim() || undefined,
      skillType: skSel.value || undefined,
      notes: noInput.value.trim() || undefined,
      memo: meInput.value.trim() || undefined,
      custom: Object.keys(custom).length ? { ...(entry.custom || {}), ...custom } : entry.custom,
    });
    rebuild();
  };
  const allInputs = [stInput, etInput, phInput, noInput, meInput, ...customInputs];
  ...
  tr.querySelectorAll("td[data-col-key]").forEach((td) => {
    ...
    td.innerHTML = "";
    if (key === "startTime") td.append(stInput);
    else if (key === "endTime") td.append(etInput);
    else if (key === "phase") td.append(phInput);
    else if (key === "skillType") td.append(skSel);
    else if (key === "notes") td.append(noInput);
    else if (key === "memo") td.append(meInput);
    else if (key === "__mits") td.textContent = "—";
    else if (key.startsWith("custom:")) {
      const colId = key.slice(7);
      const idx = (customCols || []).findIndex((c) => c.id === colId);
      if (idx >= 0) td.append(customInputs[idx]);
    }
  });
  const lastTd = tr.querySelector("td:last-child:not([data-col-key])");
  if (lastTd) {
    lastTd.innerHTML = "";
    lastTd.append(
      h("div", { class: "row-actions" },
        h("button", { class: "btn btn-primary btn-sm", onclick: save }, "✓"),
        h("button", { class: "btn btn-outline btn-sm", onclick: () => rebuild() }, "✗"),
      ),
    );
  }
  stInput.focus();
...
function renderSectionsTab(container, contentId) {
  ...
  function rebuildToolBtns() {
    ...
    sectionToolBody.append(
      ...
      h("button", {
        class: "toolbox-btn",
        onclick: () => EditLog.openModal(contentId, "sections", rebuild),
      }, "📋 수정 로그"),
      h("div", { class: "toolbox-sep" }),
      h(
        "div",
  ...
  function rebuild() {
    ...
    sections.forEach((section, idx) => {
      ...
      if (editMode && !deleteMode && !moveMode) {
        el.classList.add("edit-mode-section");
        const header = el.querySelector(".section-phase-title, .section-block-title");
        if (header) {
          header.style.cursor = "pointer";
          header.title = "클릭하여 제목 수정";
          header.addEventListener("click", () => {
            const inp = document.createElement("input");
            inp.className = "inline-input";
            inp.type = "text";
            inp.value = section.title;
            inp.style.cssText = "font-size:inherit;font-weight:inherit;width:100%;max-width:500px;font-family:inherit;";
            header.innerHTML = "";
  ...
  if (cell.type === "text") {
    ...
    if (cell.imageData) {
      ...
      if (!locked) {
        ...
        // ── 리치텍스트 캡션 에디터 ──────────────────────────
        const captionWrap = h("div", { class: "caption-rich-wrap" });
        const captionEditor = h("div", {
          class: "caption-editor",
          contenteditable: "true",
          "data-placeholder": "사진 설명 (선택)...",
        });
        captionEditor.innerHTML = cell.caption || "";
        captionEditor.onblur = () =>
          updateCellField(
            cell.id,
            row,
            contentId,
            sectionId,
            { caption: captionEditor.innerHTML },
            rebuildRows,
            false,
          );

        captionWrap.append(captionEditor);
        imgWrap.append(img, sizeRow, captionWrap, delBtn);
      } else {
        imgWrap.append(img);
        if (cell.caption) {
          const captionView = h("div", { class: "note-photo-caption-view" });
          captionView.innerHTML = cell.caption;
          imgWrap.append(captionView);
        }
      }
      contentEl = imgWrap;
    } else {
...
function renderNotes(container, contentId) {
  ...
  const toolBody = h(
    ...
    h("button", {
      class: "toolbox-btn",
      onclick: () => EditLog.openModal(contentId, "notes", rebuild),
    }, "📋 수정 로그"),
    h("div", { class: "toolbox-sep" }),
    h(
      "div",
...
function openShortcutHelp() {
  function kbSection(title, rows) {
    return h(
      "div",
      { style: "margin-bottom:20px;" },
      h("div", { style: "font-weight:700;font-size:13px;color:var(--primary);margin-bottom:8px;padding-bottom:4px;border-bottom:1px solid var(--primary-light);" }, title),
  ...
  function groupHeader(label) {
    return h("div", { style: "font-size:15px;font-weight:800;color:var(--primary);margin:20px 0 10px;padding:6px 10px;background:var(--primary-light);border-radius:8px;border-left:4px solid var(--primary);" }, label);
  }

  const body = h("div", { style: "max-height:70vh;overflow-y:auto;padding-right:4px;" },
    kbSection("⌨️ 키보드 단축키", [
      ["?", "이 도움말 열기"],
      ["Esc", "모달 / 미리보기 닫기"],
    ...
    tip("🔍", "타임라인 직업 필터", "타임라인 상단에 배치된 외생기가 여러 직업에 걸쳐 있으면 직업별 필터 탭이 나타나요. '기공사만' 등을 클릭하면 해당 직업 스킬만 표시됩니다."),
    tip("📦", "기본 데이터 불러오기", "외생기 라이브러리에서 '📦 기본 데이터 불러오기' 버튼으로 모든 직업의 주요 외생기를 한 번에 등록할 수 있어요."),
  );
