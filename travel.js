      ...
      <div class="tp-card-inner">
        ...
        <div class="tp-card-body">
          <div class="tp-card-top">
            ${t.link ? `<a href="${t.link}" target="_blank" rel="noopener noreferrer" onclick="event.stopPropagation()" class="tp-link-icon-box" style="margin-right:6px;flex-shrink:0;"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg></a>` : ''}
...
${Object.entries(dateGroups).sort().map(([date, items]) =>
  `\n[${date}]\n` + items.map(s => `  ${s.time||'--:--'} | ${s.place||''} | ${s.content||''} | ${s.category||''}${s.transport ? ' | ' + s.transport : ''}`).join('\n')
).join('\n')}
