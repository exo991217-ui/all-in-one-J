...
let bucketInlineAddOpen = false;
let tripBucketInlineAddOpen = false;
...
function renderTripBucket(items) {
  ...
  if (!filtered.length && !tripBucketInlineAddOpen) {
  ...
  // ★ 인라인 추가 폼 (가고싶은 곳)
  if (tripBucketInlineAddOpen) {
    const cats = getCategories("bucket");
    const catOpts = cats.map(c => `<option>${c}</option>`).join("");
    const addCard = document.createElement("div");
    addCard.className = "wish-cat-card";
    addCard.style.gridColumn = "1 / -1";
    addCard.innerHTML = `
      <div class="wish-cat-header">
        <span>➕</span>
        <span class="wish-cat-label">새 장소 추가</span>
      </div>
      <div style="padding:12px;display:flex;flex-direction:column;gap:8px;background:color-mix(in srgb,var(--pd,#4f46e5) 8%,#fff)">
        <div style="display:flex;gap:7px;flex-wrap:wrap">
          <input type="text" id="tbkt-add-place" placeholder="장소명 *" style="flex:2;min-width:110px;padding:6px 10px;border:1.5px solid var(--pd);border-radius:8px;font-size:.84rem;font-family:inherit"/>
          <select id="tbkt-add-cat" style="flex:1;min-width:80px;padding:6px 8px;border:1.5px solid var(--border);border-radius:8px;font-size:.82rem;font-family:inherit">
            <option value="">유형</option>${catOpts}
          </select>
          <select id="tbkt-add-season" style="flex:1;min-width:70px;padding:6px 8px;border:1.5px solid var(--border);border-radius:8px;font-size:.82rem;font-family:inherit">
            <option value="">계절</option>
            <option>봄</option><option>여름</option><option>가을</option><option>겨울</option><option>연중</option>
          </select>
        </div>
        <div style="display:flex;gap:7px;flex-wrap:wrap">
          <input type="text" id="tbkt-add-country" placeholder="나라" style="flex:1;min-width:80px;padding:6px 10px;border:1.5px solid var(--border);border-radius:8px;font-size:.82rem;font-family:inherit"/>
          <input type="text" id="tbkt-add-region" placeholder="지역" style="flex:1;min-width:80px;padding:6px 10px;border:1.5px solid var(--border);border-radius:8px;font-size:.82rem;font-family:inherit"/>
          <input type="text" id="tbkt-add-notes" placeholder="비고" style="flex:2;min-width:110px;padding:6px 10px;border:1.5px solid var(--border);border-radius:8px;font-size:.82rem;font-family:inherit"/>
        </div>
        <div style="display:flex;align-items:center;gap:10px;justify-content:flex-end;flex-wrap:wrap">
          <label style="display:flex;align-items:center;gap:5px;font-size:.82rem;color:var(--text-muted);margin-right:auto;cursor:pointer">
            <input type="checkbox" id="tbkt-add-visited" style="width:15px;height:15px;accent-color:var(--pd)"/> 방문완료
          </label>
          <button class="btn btn-ghost btn-xs" onclick="cancelTripBucketInlineAdd()" style="font-size:.78rem">취소</button>
          <button class="btn btn-primary btn-xs" onclick="saveTripBucketInlineAddRow()" style="font-size:.78rem">저장</button>
        </div>
      </div>`;
    board.appendChild(addCard);
    setTimeout(() => document.getElementById("tbkt-add-place")?.focus(), 50);
  }
}
...
// ★ 가고싶은 곳 인라인 추가 (모달 대체)
function openTripBucketInlineAdd() {
  if (isLocked) return;
  tripBucketInlineAddOpen = true;
  wishInlineEditId = null;
  renderTripBucket(allBucketItems);
}

function cancelTripBucketInlineAdd() {
  tripBucketInlineAddOpen = false;
  renderTripBucket(allBucketItems);
}

async function saveTripBucketInlineAddRow() {
  const placeName = document.getElementById("tbkt-add-place")?.value.trim();
  if (!placeName) { showToast("장소명을 입력해주세요 📍"); return; }
  const data = {
    placeName,
    category: document.getElementById("tbkt-add-cat")?.value || "기타",
    season:   document.getElementById("tbkt-add-season")?.value || null,
    country:  document.getElementById("tbkt-add-country")?.value.trim() || null,
    region:   document.getElementById("tbkt-add-region")?.value.trim() || null,
    notes:    document.getElementById("tbkt-add-notes")?.value.trim() || null,
    visited:  document.getElementById("tbkt-add-visited")?.checked || false,
  };
  await bucketRef().add(data);
  showToast("추가 완료 ⭐ (버킷플레이스에도 반영됩니다)");
  tripBucketInlineAddOpen = false;
  reloadBucketAll();
}
...
// ★ 버킷플레이스 인라인 추가 (모달 대체)
function openBucketInlineAdd() {
  if (isLocked || bucketEditMode) return;
  bucketInlineAddOpen = true;
  bucketInlineEditId = null;
  renderBucketList();
}

function cancelBucketInlineAdd() {
  bucketInlineAddOpen = false;
  renderBucketList();
}

async function saveBucketInlineAddRow() {
  const placeName = document.getElementById("bkt-add-place")?.value.trim();
  if (!placeName) { showToast("장소명을 입력해주세요 📍"); return; }
  const data = {
    placeName,
    category: document.getElementById("bkt-add-cat")?.value || null,
    season:   document.getElementById("bkt-add-season")?.value || null,
    country:  document.getElementById("bkt-add-country")?.value.trim() || null,
    region:   document.getElementById("bkt-add-region")?.value.trim() || null,
    notes:    document.getElementById("bkt-add-notes")?.value.trim() || null,
    visited:  document.getElementById("bkt-add-visited")?.checked || false,
  };
  await bucketRef().add(data);
  showToast("추가 완료 ⭐");
  bucketInlineAddOpen = false;
...
function renderBucketList() {
  ...
  if (!items.length && !bucketInlineAddOpen) {
  ...
  // ★ 인라인 추가 행
  if (bucketInlineAddOpen) {
    const catOpts = cats.map(c => `<option>${c}</option>`).join("");
    tbody.innerHTML += `<tr class="inline-edit-row" id="bkt-add-row">
      <td style="text-align:center"><input type="checkbox" id="bkt-add-visited" style="width:16px;height:16px;accent-color:var(--pd)"/></td>
      <td><select id="bkt-add-cat" style="min-width:60px"><option value="">유형</option>${catOpts}</select></td>
      <td><input type="text" id="bkt-add-country" placeholder="나라"/></td>
      <td><input type="text" id="bkt-add-region" placeholder="지역"/></td>
      <td><input type="text" id="bkt-add-place" placeholder="장소명 *" style="font-weight:600"/></td>
      <td><select id="bkt-add-season" style="min-width:60px">
        <option value="">-</option>
        ${["봄","여름","가을","겨울","연중"].map(s=>`<option>${s}</option>`).join("")}
      </select></td>
      <td><input type="text" id="bkt-add-notes" placeholder="비고"/></td>
      <td style="padding:4px;white-space:nowrap">
        <button class="btn btn-ghost btn-xs" onclick="cancelBucketInlineAdd()" style="font-size:.75rem;padding:3px 7px">취소</button>
        <button class="btn btn-primary btn-xs" onclick="saveBucketInlineAddRow()" style="font-size:.75rem;padding:3px 7px">저장</button>
      </td>
    </tr>`;
    setTimeout(() => {
      document.getElementById("bkt-add-row")?.scrollIntoView({block:"nearest"});
      document.getElementById("bkt-add-place")?.focus();
    }, 50);
  }
}
